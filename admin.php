...
$modulosValidos = ['inicio', 'investigadores', 'proyectos', 'lineas', 'noticias', 'editar_investigador', 'nuevo_investigador', 'nueva_linea', 'editar_linea', 'nuevo_proyecto', 'editar_proyecto', 'nueva_noticia', 'editar_noticia', 'inicio_config', 'nosotros_config', 'contacto_config', 'manual'];
...
<body class="flex h-screen overflow-hidden">
    ...
    <aside
        ...
        <div class="h-20 flex items-center px-6 border-b border-gray-800 bg-gray-950 gap-4">
            <img src="<?= htmlspecialchars($site_logo ?? '') ?>" alt="CIEEPE Logo"
                class="w-12 h-12 rounded-lg shadow-sm object-contain bg-white p-1 flex-shrink-0">
            <div class="min-w-0">
        ...
        <nav class="flex-1 overflow-y-auto sidebar-scroll py-6 space-y-1">
            ...
            <a href="admin.php?modulo=inicio"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'inicio' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="layout-dashboard" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Dashboard</span>
            </a>

            <h3 class="px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 mt-6">Gestión de Datos
            </h3>

            <a href="admin.php?modulo=investigadores"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'investigadores' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="users" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Investigadores</span>
            </a>

            <a href="admin.php?modulo=proyectos"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'proyectos' || $modulo === 'nuevo_proyecto' || $modulo === 'editar_proyecto' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="folder-kanban" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Proyectos</span>
            </a>

            <a href="admin.php?modulo=noticias"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'noticias' || $modulo === 'nueva_noticia' || $modulo === 'editar_noticia' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="newspaper" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Noticias</span>
            </a>

            <a href="admin.php?modulo=lineas"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'lineas' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="book-open" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Líneas de Investigación</span>
            </a>

            <h3 class="px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 mt-6">Sitio Web</h3>

            <a href="admin.php?modulo=inicio_config"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'inicio_config' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="settings" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Inicio</span>
            </a>

            <a href="admin.php?modulo=nosotros_config"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'nosotros_config' ? $activeClass : $inactiveClass ?>">
                <i data-lucide="info" class="w-5 h-5 mr-3"></i>
                <span class="font-medium text-sm">Nosotros</span>
            </a>

            <a href="admin.php?modulo=contacto_config"
                class="mx-2 px-4 py-3 flex items-center rounded-lg transition-colors <?= $modulo === 'contacto_config' ? $activeClass : $inactiveClass ?>">
        ...
        <div class="px-4 py-2 border-t border-gray-800 bg-gray-900/50">
            <a href="admin.php?modulo=manual"
                class="flex items-center px-4 py-3 rounded-lg transition-colors <?= $modulo === 'manual' ? $activeClass : $inactiveClass ?>">
                ...
                <h2 class="text-xl font-bold text-gray-800 hidden sm:block">
                    ...
                    switch ($modulo) {
                        case 'inicio':
                            ...
                            break;
                        case 'investigadores':
                            echo "Directorio de Investigadores";
                            break;
                        case 'editar_investigador':
                            echo "Edición de Perfil";
                            break;
                        case 'nuevo_investigador':
                            echo "Añadir Nuevo Investigador";
                            break;
                        case 'proyectos':
                            echo "Gestión de Proyectos";
                            break;
                        case 'nuevo_proyecto':
                            echo "Añadir Nuevo Proyecto";
                            break;
                        ...
                        case 'lineas':
                            ...
                            break;
                        case 'nueva_linea':
                            echo "Añadir Nueva Línea";
                            break;
                        case 'editar_linea':
                            echo "Editar Línea de Investigación";
                            break;
                        case 'inicio_config':
                            echo "Inicio";
                            break;
                        case 'nosotros_config':
                            echo "Nosotros";
                            break;
                        case 'contacto_config':
                            echo "Contacto";
                            break;
                        case 'manual':
